...
<div class="<?php print !empty($item->class) ? $item->class : 'item'; ?>">
  ...
  <div class="calendar <?php print $item->granularity; ?>view <?php print($item->row->node_type); ?>">
    <?php 
    $subject = theme('calendar_stripe_stripe', array('item' => $item));
    //$pattern = '/style=\"\S*\" /'; //matches the whole style
    $pattern = '/background-color:\#\S{6};/';
    preg_match($pattern, $subject, $matches);
    //$color_style = $matches[0];
    $background_color = $matches[0];
    //kpr($matches[0]); die;
    print theme('calendar_stripe_stripe', array('item' => $item)); ?>
    <div class="thecalendarcontent <?php print $item->date_id ?> contents <?php print($item->row->node_type); ?>" <?php print "style='$background_color';";?> >
